...
for t in targetlist:
    #Split test - train
    df_cut= dict_df_cuts[t].sample(frac=0.75,random_state=12)
...
plt.rcParams['xtick.labelsize'] = 25
plt.rcParams['ytick.labelsize'] = 25
g = sns.clustermap(ccm_eccm.fillna(0), cbar=False, col_cluster=False, row_cluster=False,  linewidths=0.1, cmap='Blues', xticklabels=True, yticklabels=True  )
g.ax_row_dendrogram.set_visible(False)
...
plt.rcParams['xtick.labelsize'] = 25
plt.rcParams['ytick.labelsize'] = 25
g = sns.clustermap(df_cor, cbar=False, vmin=-1, vmax=1, col_cluster=False, row_cluster=False,  linewidths=0.1, cmap='coolwarm', xticklabels=True, yticklabels=True,\
                   )
g.ax_row_dendrogram.set_visible(False)
g.ax_col_dendrogram.set_visible(False)
...
#create dict as a dummy function
dict_dummy = {}
scores = []
X_train = []

for i in df_de_max.values.tolist():
    dict_dummy[str(i[:-2])] = i[-2]
    scores.append(i[-2])
    X_train.append(i[:-2]) 
...
Y = np.zeros([scores.shape[0]])

def shapFunc(v):
    print(v)
    v= [str(int(round(int(i)))) for i in v]
    return dict_dummy[str(list(v))]

X = df_de_max[list(df_de_max.columns)[:-2]].astype(int).values.tolist()
X = np.asarray(X)

for i, x in enumerate(X):
        Y[i] = float(shapFunc(x))  
...
def f(x):
    ...
    for i in x:
        ...
        try:
            v= [str(int(round(int(j)))) for j in i]
            allX.append(dict_dummy[str(list(v))])
        except:
            allX.append(np.mean(list(dict_dummy.values())))
    return np.asarray(allX)


df = pd.DataFrame(X, columns=list(df_de_max.columns)[:-2])

cols, _ = fix_labels(list(df.columns))
df.columns = cols
...
shap.plots.beeswarm(shap_values=shap_values, max_display=13)
fig.savefig(BaseFolder+"swarm_plot_SHAP.png", bbox_inches='tight', dpi=600)

fig = plt.figure()
plt.axes().set_facecolor('white')
g = shap.summary_plot(shap_values, df, plot_type="bar")
fig.savefig(BaseFolder+"summary_plot_SHAP.png", bbox_inches='tight', dpi=600)
