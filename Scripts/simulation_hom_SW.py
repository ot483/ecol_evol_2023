...
for i in All_CCM_dfs:
    if (len(i[2]) > 0):
        try:
            if (i[1]["x1_mean"][:50][-5:].mean() >= 0.01):
            #if (i[-2] == True) and (i[-1] == True):
                i[1]["x1_mean"][:50].plot()
                print(i[2][0][2] + ' ' + i[2][0][3])
                CausalFeatures.append([i[2][0][2], i[2][0][3],  i[1]["x1_mean"][:50][-5:].mean()])
        except:
            print('e')

df_CausalFeatures2 = pd.DataFrame(data=CausalFeatures, columns=['x1', 'x2', 'Score'])

Features = list(df_CausalFeatures2['x1'].unique()) + list(df_CausalFeatures2['x2'].unique())
Features = list(set(Features))

Features = Features + cyanolist
Features = list(set(Features))
...
plt.savefig(BaseFolder+"causalityNet_sim_homogenous.png")
...
df_CausalFeatures2.to_csv(BaseFolder+'df_CausalFeatures2_sim_homogenous.csv')
...
for j in edges:
 ...
 ###
    ...
    print(str(i))
    print("*************************************")    
    df_.plot(legend=False).get_figure().savefig(BaseFolder+"eccm_sim_"+str(j[0])+"_"+str(j[1]+".png"))       
 


with open(BaseFolder+'AllECCM_ccm_dag_sim_homogenous.pickle', 'wb') as handle:
...
df_CausalFeatures2 = pd.read_csv(BaseFolder+'df_CausalFeatures2_sim_homogenous_curated.csv')
...
df_violin = df_CausalFeatures2[df_CausalFeatures2['x2'].isin(cyanolist)]
...
while s == 0:
    try:
        ...
        elif (cycles[-1][0] in cyanolist) and not (cycles[-1][1] in cyanolist):
            G_dag_tmp.remove_edge(cycles[-1][0], cycles[-1][1])
        elif (cycles[-1][1] in cyanolist) and not (cycles[0][1] in cyanolist):
            G_dag_tmp.remove_edge(cycles[0][0], cycles[0][1])    
        elif (cycles[0][1] in cyanolist) and not (cycles[-1][1] in cyanolist):
           G_dag_tmp.remove_edge(cycles[-1][0], cycles[-1][1]) 
        elif (cycles[0][0] in cyanolist):
           G_dag_tmp.remove_edge(cycles[-1][0], cycles[-1][1])          
        else:
            G_dag_tmp.remove_edge(cycles[0][0], cycles[0][1])
    except:
...
while s == 0:
    try:
        remove = [node for node,degree in dict(G_dag_tmp.out_degree()).items() if (degree == 0) and not (node in cyanolist)]
