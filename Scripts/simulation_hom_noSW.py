...
with open(BaseFolder+'sim_CCM_homogenousNoWindow.pickle', 'wb') as handle:
...
with open(BaseFolder+'sim_CCM_homogenousNoWindow.pickle', 'rb') as handle:
...
plt.savefig(BaseFolder+"causalityNet_sim_homogenous_noWindow.png")
...
#Learn probabilities of a given network structure


import bnlearn as bn

df_CausalFeatures2.to_csv(BaseFolder+'df_CausalFeatures2_sim_homogenous_noW.csv')
...
for j in edges:
 ...
 ###
    ...
    print(str(i))
    print("*************************************")    
    df_.plot(legend=False).get_figure().savefig(BaseFolder+"eccm_sim_homogenous_noW_"+str(j[0])+"_"+str(j[1]+".png"))       
 


with open(BaseFolder+'AllECCM_ccm_dag_sim_homogenous_noW.pickle', 'wb') as handle:
    pickle.dump(allECCM, handle, protocol=pickle.HIGHEST_PROTOCOL)




df_CausalFeatures2 = pd.read_csv(BaseFolder+'df_CausalFeatures2_sim_homogenous_noW_curated.csv')
