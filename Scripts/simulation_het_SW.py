...
amplified_dfs = amplifyData(df=concated_, subSetLength=100, jumpN=50)
...
with open(BaseFolder+'sim_hetW_CCM.pickle', 'wb') as handle:
...
with open(BaseFolder+'sim_hetW_CCM.pickle', 'rb') as handle:
...
for counti, i in enumerate(All_CCM_dfs):
    ...
    try:
        ...
        if   (df_Scores["x1_mean"][l:].mean() >= df_Scores["x2_mean"][:l].mean() and \
             (df_Scores["x1_mean"][-10:].mean() >= 0.01)):
            print('dd')
            All_CCM_dfs[counti].append(True)
            
            print(All_CCM_dfs[counti][-2][-1][-10]+' ' +All_CCM_dfs[counti][-2][-1][-5])
        else:
...
for i in All_CCM_dfs:
    if (len(i[2]) > 0):
        try:
            if (i[1]["x1_mean"][:50][-10:].mean() >= 0) :
            #if (i[-2] == True) and (i[-1] == True):
                i[1]["x1_mean"][:50].plot()
                print(i[2][0][2] + ' ' + i[2][0][3])
                CausalFeatures.append([i[2][0][2], i[2][0][3],  i[1]["x1_mean"][:50][-10:].mean()])
        except:
            print('e')
...
plt.savefig(BaseFolder+"causalityNet_sim_hetW.png")
...
Cols = [i.replace('_', ' ') for i in Cols ]#+ list(Dict_groups.values())]
df = concated_[Cols]
...
df_CausalFeatures2.to_csv(BaseFolder+'df_CausalFeatures2_sim_hetW.csv')
...
for j in edges:
    ...
    for i in list(range(-20, 20, 1)): 
        ...
        #for k in amplified_dfs:
            x1 = df_upsampled_proc[j[0]].values.tolist() 
            x2 = df_upsampled_proc[j[1]].values.tolist() 
 ...
 ###
    ...
    print(str(i))
    print("*************************************")    
    df_.plot(legend=False).get_figure().savefig(BaseFolder+"eccm_sim_hetW_"+str(j[0])+"_"+str(j[1]+".png"))       
 


with open(BaseFolder+'AllECCM_ccm_dag_sim_hetW.pickle', 'wb') as handle:
...
with open(BaseFolder+'AllECCM_ccm_dag_sim_hetW.pickle', 'rb') as handle:
...
df_CausalFeatures2 = pd.read_csv(BaseFolder+'df_CausalFeatures2_sim_hetW_curated.csv')
...
df_concated_fixed_outlayers = concated_.copy()
df_concated_fixed_outlayers = df_concated_fixed_outlayers.reset_index(drop=True)
df_concated_fixed_outlayers = df_concated_fixed_outlayers.dropna()[:-1]
...
learned_dags_djmat = learned_dags_djmat[['y1', 'y2', 'y3', 'y4']]
learned_dags_djmat = learned_dags_djmat.T[['y1', 'y2', 'y3', 'y4']].T
...
df_cor = df_cor[['y1', 'y2', 'y3', 'y4']]
df_cor = df_cor.T[['y1', 'y2', 'y3', 'y4']].T

sns.set(font_scale=3)
g = sns.clustermap(df_cor, col_cluster=False, row_cluster=False,  linewidths=0.1, cmap='coolwarm', xticklabels=True, yticklabels=True)
g.ax_row_dendrogram.set_visible(False)
g.ax_col_dendrogram.set_visible(False)
plt.show()
